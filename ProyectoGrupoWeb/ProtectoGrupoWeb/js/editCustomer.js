  ...
  peticionHTTP
    ...
    .then((cliente) => {
      ...
      for (const pedido of cliente.lista) {
        ...
        elemento = document.createElement("td");
        elemento.innerHTML =
          `<button class="btn btn-warning ml-2" onclick="editaCliente(${pedido.id})"><i class="bi bi-pencil"></i></button>` +
          `<button class="btn btn-danger ml-2"  onclick="borrarCliente(${pedido.id})"><i class="bi bi-circle"></i></button>` +
          `<button class="btn btn-success ml-2"  onclick="verOrden(${pedido.id})"><i class="bi bi-eye"></i></button>`;
        fila.appendChild(elemento);
